...
static void sort_instructionsx(int ** instructions, double ** prefactors, const int nr_instructions, 
    const int step);
...
static void print_T3NS_instructions(int * const instructions, double * const prefactors, 
    int * const hss, const int nr_instructions, const int bond, const int is_left);
...
int get_next_unique_instr(int * const curr_instr, const struct instructionset * const instructions)
{
   ...
   * instructions->hss_of_new should also be different from prev instruction. */
  if (*curr_instr == -1) {
    ++*curr_instr;
    return 1;
  } else {
    const int old1 = instructions->instr[instructions->step * *curr_instr];
    const int old2 = instructions->instr[instructions->step * *curr_instr + 1];
    ...
    const int nr_instructions, const int bond, const int is_left, const char kind)
{
  switch(kind)
  {
    ...
    case 'm':
      ...
      break;
    case 't':
      print_T3NS_instructions(instructions, prefactors, hss, nr_instructions, bond, is_left);
      break;
    default:
