...
{
  const int * irreps = hdat.su2 ? &irreps_of_hamsymsec_SU2[0][0] 
    : &irreps_of_hamsymsec[0][0];
...
void NN_H_get_string_of_rops(char buffer[], const int ropsindex, const int bond, const int is_left, 
    const char o)
{
  assert(0);
}

void NN_H_get_string_of_siteops(char buffer[], const int siteindex, const int site)
{
  assert(0);
}
