...
void init_ops_types( void )
{
  ...
  for( is_left = 1 ; is_left >= 0 ; --is_left )
  {
    for( i = 0 ; i < netw.nr_bonds - 1 ; ++i )
    {
      ...
      ops_expanded_array   = NULL;
      fetch_expand_ops( &instructions, &prefactors, &nr_instructions, bond, is_left, 1 );
      ops_exp[bond * 2 + is_left] = make_ops_type(instructions, nr_instructions, bond, is_left, 1 );
      safe_free( instructions );
      safe_free( prefactors );
#endif
    }
...
static int need_ops2( const int bond, const int is_left, const int psite1, const int psite2 )
{
  ...
  else
  {
    ...
    cnt = site_is_left_of_bond( bonds[ 0 ], psite1 ) ? 0 : 1;
